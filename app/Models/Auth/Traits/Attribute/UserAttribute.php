...
trait UserAttribute
{
    ...
    public function getSocialButtonsAttribute()
    {
        ...
        if (auth()->user()->can('manage users'))
            ...
            foreach ($this->providers as $social) {
                $accounts[] = '<i class="fa fa-'.$social->provider.'"></i>';
            }
    ...
    {
        if(auth() && (auth()->user()->hasRole('administrator')))
    ...
    {
        if ($this->id == 1) return '';
        if (! auth()->user()->can('authorize users')) return '';
        if ($this->id != auth()->id()) {
            switch ($this->active) {
                case 0:
    ...
    {
        if (! auth()->user()->can('confirm users')) return '';
    ...
    public function getActionButtonsAttribute()
    {
        ...
        if ($this->trashed() && auth()->user()->can('manage users')) {
            return '
